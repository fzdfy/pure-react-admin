...
import type { MenuListRequest, TypesListMenuResp } from '@/request'
...
const getMenuList = () => {
  return getTableData((params?: MenuListRequest) => pureApi.menuList(params))
}

const MenuList: FC = () => {
  const ref = useRef<ActionType>()

  const showDeleteMenuModal = (record: any) => {
    NiceModal.show(AntdModal, {
      title: <Typography.Title>删除菜单</Typography.Title>,
      children: <Typography.Text>确认删除{record.name}?</Typography.Text>,
      onOk: () => {
        pureApi.menuDelete({ body: { id: record.id } }).then(() => {
          ref.current?.reload()
        })
      },
    })
  ...
  return (
    <PageContainer>
      <ProTable<TypesListMenuResp, MenuListRequest>
        actionRef={ref}
        rowKey='id'
        columns={columns}
        request={getMenuList()}
          ...
          <Button
            ...
            onClick={() => {
              const drawer = formDrawer({
                ...
                onOk: (values) => {
                  return pureApi.menuAdd({ body: values }).then(() => {
                    ref.current?.reload()
                  })
                },
              })
